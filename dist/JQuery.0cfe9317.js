...
window.jQuery = function (selectorOrArray) {
  //接受selectorOrArray（选择器或数组）
  var elements; //申明一个空的elements

  if (typeof selectorOrArray === 'string') {
    //判断：如果selectorOrArray的类型是一个字符串
    elements = document.querySelectorAll(selectorOrArray); //获取到所有元素们并赋值给elements
  } else if (selectorOrArray instanceof Array) {
    //判断：如果selectorOrArray的类型是一个数组
  ...
  return {
    ...
    children: function children() {
      var array = []; //声明一个空对象用于放元素的孩子们
      //找到所有孩子们，并添加到数组中

      this.each(function (node) {
        if (array.indexOf(node.children) === -1) {
          //判断：如果数组中没有，就添加进去
          array.push.apply(array, _toConsumableArray(node.children)); //'...'是展开操作符
        }
      });
      return jQuery(array); //返回api
    }
...
},{}]},{},["C:/Users/Administrator.USER-20160425SJ/AppData/Local/Yarn/Data/global/node_modules/parcel/src/builtins/hmr-runtime.js","JQuery.js"], null)
